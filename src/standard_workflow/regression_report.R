...
beta2   <- read.csv('~/Documents/workspace/phospho_network/script_files/analysis_rppa/mapk/beta_matrix_nop.csv',as.is = T)
result2 <- read.csv('~/Documents/workspace/phospho_network/script_files/analysis_rppa/mapk/result_matrix_nop.csv',as.is = T)
...
for (unique_id in site_ids){
  ...
  beta_matrix <- t(beta_matrix[,-(1:2)])
  colnames(beta_matrix) <- beta_matrix$predictor
  boxplot(beta_matrix,cex.axis=0.6,las = 2, main = 'RPPA coefficients in CV')
  
  beta_matrix2 <- t(beta_matrix2[,-(1:2)])
  colnames(beta_matrix2) <- beta_matrix2$predictor
  boxplot(beta_matrix2,cex.axis=0.6,las = 2, main = 'No individual penalty RPPA coefficients in CV')
}
