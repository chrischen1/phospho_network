...
# 4.Construct the random forest regression model: PeptideA_site1 ~ PeptideA_site1 + PeptideA_site2...
...
# 2.beta_matrix is the table for coefficient from different testsets.(not used in random forest model)
...
# Set parameters
result_path       = '~/Documents/workspace/phospho_network/script_files/TCGA_analysis/random_forest'
result_outfiles   = c('result_matrix_rf_no_network.csv','beta_matrix_rf_no_network.csv')

k                 = 0.01  # a parameter to define the extend of predictors by keep nodes receive more heat than k*(heat_response_CNV)
alphas            = c(1)  # some hyperparameter of random forest could be used here 
outerfold         = 10
innerfold         = 9
...
data_prepare <- function(test_name,mdata,mut_data,cnv_data,rna_data,predictors,i_penalty){
  ...
  mut_x <- matrix(0,nrow = ncol(mut_data),ncol = 0)
  gene_types <- c()
  for (gene_name  in mut_symbol){
    mut_gene <- t(mut_data[gene_name,])
    mut_types <- unique(unlist(strsplit(as.character(mut_gene[mut_gene!='']),split = ';')))
    for(mut in mut_types){
      gene_types <- c(gene_types,paste(gene_name,mut,sep = '.'))
      mut_gene_type_ind <- grep(mut,mut_gene)
      mut_x_new <- rep(0,nrow(mut_x))
      mut_x_new[mut_gene_type_ind] <- 1
      mut_x <- cbind(mut_x,mut_x_new)
    }
    colnames(mut_x) <-gene_types
  }
  ...
  data_model$site_id <- unname(test_name)
  return (data_model)
}
...
nestcv <- function(model_data,alphas,lambdas = NULL,outerfold = 3,innerfold = 10){
  ...
  for (i in 1:outerfold){
    ...
    # inner loop
    for(alpha in alphas){
      cv_model <- randomForest(model_data$x[-test_list[[i]],],model_data$y[-test_list[[i]]])
    ...
    outer_test_pred <- predict(best_model,newdata = test_x)
    result_matrix <- rbind(result_matrix,cbind(rep(model_data$site_id,length(outer_test_set)),outer_test_set,alpha,unname(outer_test_pred),model_data$y[outer_test_set],rep(0,length(outer_test_set))))
  }
  ...
  return(list('matrix' = result_matrix,'best_outer_q2' = outer_q2))
}
...
mut_missing_data <- matrix('',ncol = length(mut_missing),nrow = nrow(mut_data))
...
predictors_all <- rep(1,length(node_all))
names(predictors_all) <- node_all
# regression alalysis
result_matrix <- matrix(0,nrow = 0,ncol = 6)
colnames(result_matrix) <- c('gene_site', 'test_set', 'alpha', 'predict_value', 'true_value', 'best_outer_q2')
