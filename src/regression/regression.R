...
predictor_construct <- function(test_node_valid,pred_choose,network){
  if (pred_choose == 'connect'){
    bfs_dist <- get.all.shortest.paths(graph_from_adjacency_matrix(t(network)),from = test_node_valid)$nrgeo
  ...
  }else {warning('unregonized predictor selection method')}
...
data_prepare <- function(test_name,mdata,mut_data,cnv_data,rna_data,mis_mut,predictors,i_penalty,scale_method = '0-1'){
  # add rna info
  rna_predictors <- predictors[grep('_total',names(predictors))]
  rna_symbol <- gsub('(.+)_total','\\1',names(rna_predictors))
  rna_symbol <- rna_symbol[rna_symbol %in% rownames(rna_data)]
  rna_x <- t(rna_data[rna_symbol,])
  if(ncol(rna_x)>0){
    colnames(rna_x) <- paste(colnames(rna_x),'total',sep = '_')
  }
  # add cnv info
...
inner_svm <- function(model_data,train_set,innerfold){
...
nestcv_svm <- function(model_data,ncore=2,outerfold,innerfold){
  ...
  best_models <- foreach(i = 1:outerfold, 
                           ...
                           .export =c('outerfold','inner_svm'),
                           .packages = 'e1071')  %dopar% inner_svm(model_data,train_list[[i]],innerfold)
  stopCluster(cl)
  #end on outer loop
  for(i in 1:length(best_models)){
    ...
    parameter <- best_models[[i]]$bestk
    datax <- model_data$x[train_list[[i]],]
    beta_eval <- t(best_models[[i]]$coefs) %*% datax[best_models[[i]]$index,]
    beta_eval <- beta_eval[1,]  #need new implementation here, maybe
...
nestcv_rf  <- function(model_data,ncore=2,outerfold,innerfold){
  sample_list <- sample_prepare(nsample = nrow(model_data$x),fold = outerfold)
  result_matrix <- matrix(0,nrow = 0,ncol = 6)
  colnames(result_matrix) <- c('gene_site', 'test_set', 'parameter', 'predict_value', 'true_value', 'best_outer_q2')
  beta_matrix_raw   <- matrix(0,nrow = length(model_data$penalty),ncol = outerfold)
 ...
 #need new implementation here, maybe
    result_matrix <- rbind(result_matrix,cbind(model_data$site_id,test_list[[i]],parameter,outer_test_pred,model_data$y[test_list[[i]]],0))
    beta_matrix_raw[names(beta_eval),i] <- beta_eval
  }
  pred_all <- as.numeric(result_matrix[,'true_value'])
...
nestcv_en  <- function(model_data,ncore=2,outerfold,innerfold,alphas = seq(0,1,0.05)){
  sample_list <- sample_prepare(nsample = nrow(model_data$x),fold = outerfold)
  result_matrix <- matrix(0,nrow = 0,ncol = 6)
  colnames(result_matrix) <- c('gene_site', 'test_set', 'parameter', 'predict_value', 'true_value', 'best_outer_q2')
  beta_matrix_raw   <- matrix(0,nrow = length(model_data$penalty),ncol = outerfold)
...
if(length(test_node_valid)>0){
  if(pred_choose == 'hf'){
    ...
    predictors <- predictor_heat(test_node_valid,hi_matrix,k)
  }else if (pred_choose == 'all'){
      predictors_name <- c(paste(rownames(mut_data),'mutation_misc',sep = '_'),paste(rownames(rna_data),'total',sep = '_'),
  ...
  if (!is.null(model_data)){
    if(algos == 'rf'){model_result  <- nestcv_rf(model_data = model_data,outerfold = outerfold,innerfold = innerfold,ncore = ncore)}
    else if(algos == 'en'){model_result  <- nestcv_en(model_data = model_data,outerfold = outerfold,innerfold = innerfold,ncore = ncore,alphas = alphas)}
    else if(algos == 'svm'){model_result  <- nestcv_svm(model_data = model_data,outerfold = outerfold,innerfold = innerfold,ncore = ncore)}
    else{warning('unregonized algorithm!')}
    result_matrix_all <- rbind(result_matrix_all,model_result$matrix)
    beta_matrix_all   <- rbind(beta_matrix_all,model_result$beta_matrix)
  }
  result_matrix_all <- cbind(result_matrix_all,'predictors_given' = length(predictors))
