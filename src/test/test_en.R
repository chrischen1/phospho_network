...
inner_en1 <- function(model_data,train_set,innerfold,alphas = alphas){
  datax <- model_data$x[train_set,]
  datay <- model_data$y[train_set]
  # inner loop
  best_alpha <- 0
  ...
  best_model <- glmnet(datax,datay,alpha = best_alpha,lambda = 0,penalty = model_data$penalty)
  best_model$alpha <- best_alpha
  return(best_model)
}

inner_en2 <- function(model_data,train_set,innerfold,alphas = alphas){
  datax <- model_data$x[train_set,]
  datay <- model_data$y[train_set]
  # inner loop
  best_alpha <- 0
  ...
  cv_model <- cv.glmnet(datax,datay,nfolds = innerfold,penalty.factor = model_data$penalty,alpha = best_alpha,grouped=FALSE)
  best_model <- glmnet(datax,datay,alpha = best_alpha,lambda = cv_model$lambda.min,penalty = log(model_data$penalty+0.0001))
  best_model$alpha <- best_alpha
  return(best_model)
}
inner_en3 <- function(model_data,train_set,innerfold,alphas = alphas){
  datax <- model_data$x[train_set,]
  datay <- model_data$y[train_set]
  # inner loop
  best_alpha <- 0
...
nestcv <- function(model_data,ncore=2,outerfold,innerfold,alphas = seq(0,1,0.05),inner_en){
  ...
  for(test_name in response_list[,1]){
    ...
    if(length(test_node_valid)>0){
      ...
      if (!is.null(model_data)){
        # model_result  <- nestcv(model_data = model_data,outerfold = outerfold,innerfold = innerfold,ncore = ncore,alphas = alphas,inner_en=inner_en)
        # q2_array <- c(q2_array,unique(model_result$matrix[,6]))
      }
    }
  }
  return(q2_array)
...
test_ifs <- c(inner_en1,inner_en2,inner_en3)
for(i in 1:length(test_ifs)){
  inner_en = test_ifs[[i]]
  q2_list[[i]] <- en_test(inner_en)
}
# q2_table <- do.call(cbind,q2_list)
# write.csv(q2_table,'en_test.csv')
