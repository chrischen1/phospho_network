...
mutation_matrix <- matrix(0,nrow = length(gene_list),ncol = length(sample_list))
colnames(mutation_matrix) <- sample_list
rownames(mutation_matrix) <- gene_list

for (i in 1:nrow(mut_data)){
  mutation_matrix[mut_data$Hugo_Symbol[i],mut_data$Tumor_Sample_Barcode[i]] <- 1
}

write.csv(mutation_matrix,'~/Documents/workspace/phospho_network/script_files/TCGA_analysis/mutation_matrix.csv')
