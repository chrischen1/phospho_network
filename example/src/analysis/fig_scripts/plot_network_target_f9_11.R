...
plot_config <- function(beta_table,output_path,network){
  ...
  for(test_name in test_names){
    ...
    if(length(test_node_valid) > 0){
      print(test_name)
      png(filename = paste(output_path,'/',test_node_valid,'.pdf',sep = ''),width = 1280,height = 960)
...
plot_config(beta_table,output_path,network)
